...
# on mac
setwd("/Users/rhalenathomas/Documents/scRNAclubMcGill/workshop2023/")
# put the pathway to your files here


# read in the data objects if starting from save files
adolescent_data_seurat <- readRDS(" adolsecentSeurate.RDS") %>% RenameCells(add.cell.id = "Adol_")
adult_data_seurat <- readRDS(" adultSeurate.RDS") %>% RenameCells(add.cell.id = "Adult_")
...
Save the integrated object or read in the Integrated object for the next step.

```{r}
# remove # comment from the code you want to run
#saveRDS(integrated_seurate,"IntegratedSeurat.RDS")
...
```{r}
# remove # comment from the code you want to run
#saveRDS(integrated_seurate,"IntegratedSeuratGraphs.RDS")
#integrated_seurate <- readRDS("IntegratedSeuratGraphs.RDS")
...
```{r}
# remove # comment from the code you want to run
#saveRDS(integrated_seurate,"IntegratedSeuratClusters.RDS")
#integrated_seurate <- readRDS("IntegratedSeuratClusters.RDS")
...
Now that we have the an different forms of information to annatote the cells we need to decide on the annotations to add, then add them into the data object.
