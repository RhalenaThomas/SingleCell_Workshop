...
adolescent_data_seurat <- readRDS(paste(output_path,"adolsecentSeurate.RDS"))
adult_data_seurat <- readRDS(paste(output_path,"adultSeurate.RDS"))
...
library("Matrix")
# Convert sparse matrix to dgCMatrix object
dgCMat_ref <- t(Matrix(ref@assays$RNA@counts, sparse = TRUE))
# Get metadata as a dataframe
metadata_df <- data.frame(cell_type = ref$WGCNAcluster)
rownames(metadata_df) <- colnames(ref@assays$RNA@counts)

# Combine count matrix and metadata into one matrix
combined_mat <- cbind(dgCMat_ref, metadata_df)
#convert to matrix
# Convert combined matrix to a matrix object
combined_mat <- as.matrix(combined_mat)

# Convert matrix to a dgCMatrix object
combined_dgCMatrix <- as(combined_mat, "dgCMatrix")

# convert the integrated clustered object into 
query_expr_mat <- as.sparse(integrated_seurate@assays$RNA@counts)
dgCMat_query <- as(query_expr_mat, "dgCMatrix")
