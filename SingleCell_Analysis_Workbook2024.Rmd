...
library(anndata) # for reading in the listr data 
library(Matrix)
library(scclusteval)
library(mclust)
library(MAST)
...
# this is the scanpy object 
lister_data <- read_h5ad("ListerLab/Processed_data_RNA-all_full-counts-and-downsampled-CPM.h5ad")
lister_data
class(lister_data)
...
### NOTE I had issues converting by this method
mat_t <- t(lister_data$X)
lister_data_seu <- CreateSeuratObject(counts = mat_t, meta.data = lister_data$obs)
...
#Convert("ListerLab/Processed_data_RNA-all_full-counts-and-downsampled-CPM.h5ad", dest = "h5seurat", overwrite = TRUE)
lister_seu <- LoadH5Seurat("ListerLab/Processed_data_RNA-all_full-counts-and-downsampled-CPM.h5seurat")
...
Now we will compute cell-type specific, cell-based differential gene expression analysis between our sibjucts groups (age 14 & age 41)
We will begin by running an interactive example with Microglia. Later on in the workshop we will run a loop for all of the cell types. 
