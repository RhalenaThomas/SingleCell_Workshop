...
In Seurat V5 we now have the two datasets show as separate layers. 
...
Look at the UMAP and find clusters in the merge only object
...
Lets compare a couple of integration methods: The Seurat method (recipical pca) and harmony 
...
Look at the merge compared to two methods of integrations

```{r, fig.width= 15}

wrap_plots(c(pMerge, pRPCA, pHarmony), ncol = 3, byrow = F)
...
2) Reduction option (integrated or not)
...
```
15 PCs is a good cutoff here. 
We will use the integrated RPCA reduction
For the starting neighbours in FindNeighbors this the k.param and in Find 
...
integrated_seurat <- FindNeighbors(integrated_seurat, dims = 1:15, reduction = "integrated.rpca", k.param = 95)
...
# Annotation
Now we choose a resolution to annotate. We are expecting microglia, astrocytes, oligodendrocytes and OPCs. We will choose resolution 0.4
...
DoHeatmap(integrated_seurat, features = top5$gene, size = 3, angle = 90, group.by = "RNA_snn_res.0.4", slot = "data")
